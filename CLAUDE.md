...
- **Three providers must stay normalized.** When adding or changing provider behavior, preserve the unified contract across `ClaudeProvider`, `CodexProvider`, `CopilotProvider` (session flags, streaming chunk types, permission flags). See the comparison table in `README.md`. Session resume works on every platform including Windows: the earlier Windows `-p` + `--resume` hang (issue #4) was an interactive-stdin wait, and no spawn path leaves stdin open for the CLI to read from — `run_subprocess_sync`/`run_subprocess_async` always set `DEVNULL`, or `PIPE` written and closed for prompts over 8,000 UTF-8 bytes — so the former Windows `supports_sessions=False` guard was removed (issue #27, verified end-to-end on Windows 11 / Claude Code 2.1.220). Keep that stdin invariant when touching `providers/base.py`: it is what makes issue #4 unreachable.
