...
역할을 과장 없이 말하면 이렇습니다. 앱에 에이전트 백엔드가 필요할 때
일반적인 경로는 hosted API 위에 에이전트 루프를 직접 만드는 것입니다.
`agentcli`는 반대 경로를 택합니다: 사용자가 이미 설치하고 로그인한 —
대개 정액 구독으로 인증된 — agentic CLI를 그 백엔드로 재사용합니다.
하네스(tool loop, 스킬, 프로젝트 지시문, 세션 히스토리)는 CLI가 그대로
소유하고, 이 라이브러리는 그 주위의 임베딩 클라이언트일 뿐입니다.

이 선택에는 두 가지 한계가 따릅니다. 개인·사내 자동화에 맞는 도구이지,
타인의 구독으로 제3자에게 서비스를 제공하는 경로가 아닙니다. 그리고 CLI
인터페이스는 버전드 API보다 빨리 바뀝니다 — 그 변동을 안정적인 Python
계약 뒤에서 흡수하는 것이 바로 이 라이브러리가 존재하는 이유이자 유지보수의
본질입니다.
