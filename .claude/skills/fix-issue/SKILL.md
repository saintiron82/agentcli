---
name: fix-issue
description: agentcli GitHub 이슈를 독립 fix 브랜치에서 재현 → 원인 파악 → 수정 → 배포 준비 → 병합·태그까지 6단계로 처리한다. 인자로 이슈 번호를 받는다 (예 `/fix-issue 4`). Use when fixing reported bugs in agentcli end-to-end on an isolated branch, from issue triage through merged tag. Triggers: '이슈 수정', '버그 수정', '재현해서 고쳐', 'fix issue', 'reproduce and fix'.
...
스킬 종료 전 `/tmp/repro_${ARGUMENTS}.py` 등 임시 디버깅 파일을 제거하라. 회귀 테스트(`tests/test_issue_${ARGUMENTS}.py`)는 커밋에 포함된다.
