...
- 새 호출은 새 CLI 세션을 만들거나 기존 `session_id`로 resume합니다.
...
현재 테스트는 session routing, async/streaming parity, alias resolution, health check, drift detection, usage aggregation, profile materialization, SQLite session persistence, Codex/Copilot JSONL parsing을 다룹니다.

## 릴리즈

- 현재 릴리즈: `0.4.0`
- 릴리즈 노트: [docs/releases/v0.4.0.ko.md](docs/releases/v0.4.0.ko.md)
