...
*같은 한 줄 프롬프트, MCP 서버 2개 + skill 28개 깔린 개발 머신, Claude Code
2.1.229.
...
`explicit`/`isolated` 의 툴 배선: `allowed_tools` 의 빌트인 이름은
`--tools`(정의 allowlist — 턴당 컨텍스트를 실제로 ~1/3 줄이는 것이 실측됨)로
가고, `mcp__*` 이름이 하나라도 섞이면 목록 전체가 `--allowedTools`(권한
게이트)로 가서 자기 MCP 서버의 툴을 좁히는 패턴이 계속 동작한다.
`mcp_config` 는 `explicit` 에서 동작한다 — 그것이 이 티어의 존재 이유다
(`--setting-sources ""` 는 ambient 상속을 끊되 명시적 `--mcp-config` 는
존중한다; 둘 다 죽이는 `--safe-mode` 와의 결정적 차이) — `isolated`/`lean`
...
내 상황에 맞는 답만 바로 원하면 [요리책](docs/cookbook.ko.md)에 케이스별
복붙 처방(대량 배치, 임베딩, 캐싱, Windows, 병렬, 업그레이드)이 있다. 이
섹션은 그 처방들의 원리를 설명한다.
...
그 A/B 를 위한 근거 앵커(2026-08 조사): thinking 하드 오프
(`MAX_THINKING_TOKENS=0`)는 독립 벤치마크들에서 재현적으로 **정확도
10–15%p** 를 깎는다 — 쓴다면 effort 제어가 없는 모델의 순수 분류/추출
전용으로만. 공식 effort 앵커: **Sonnet 5 의 `medium` ≈ Sonnet 4.6 의
`high`**, Opus 4.5 의 `medium` 은 Sonnet 4.5 최고 SWE-bench 점수와
동점이면서 출력 토큰 76% 절감 — 즉 `medium` 이 가장 근거가 탄탄한
스텝다운이지만, Anthropic 자체 가이드도 **자기 eval 로 품질 유지를 확인한
후에만** 내리라고 조건을 건다. `low` 는 문서상 "일부 능력 저하"가 명시돼
있고 비코딩 챗 용도로만 권고된다. effort 는 하드 캡이 아니라 행동
신호라서, 낮은 수준에서도 정말 어려운 문제에는 모델이 여전히 thinking 을
한다.
...
`lean` 은 가장 좁은 환경 티어다(`env="lean"`; 위 *환경 티어* 참고).
...
| claude 전용 옵션 | `env`(티어), `lean`, `isolated`, `partial_messages` | — | — | — |
...
테스트 스위트는 session routing, async/streaming parity, alias resolution, health check, drift detection, usage aggregation, profile materialization, SQLite session persistence, 같은 conversation 동시 호출 직렬화, lean/debug 커맨드 빌딩, partial-message 토큰 스트리밍, 프로세스 그룹 teardown, Codex/Copilot JSONL parsing을 다룹니다.
